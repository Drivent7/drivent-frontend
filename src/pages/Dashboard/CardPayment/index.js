  ...
  return (
    <>
      ...
      <CardPayments>
        ...
        <header>
          <h1>{complement(ticket)}{ticket?.TicketType.name}</h1>
          <h2>R$ {ticket?.TicketType.price}</h2>
        </header>
        <p>Pagamento</p>
        <section>
          {finalPayment ? <nav><img src={image} alt=''></img><aside><h3>Pagamento confirmado!</h3><h4>Prossiga para escolha de hospedagem e atividades</h4></aside></nav> : <CardComponent />}
        </section>
      </CardPayments>
    </>
  );
